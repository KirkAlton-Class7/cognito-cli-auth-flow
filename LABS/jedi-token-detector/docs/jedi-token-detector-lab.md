...
```bash
mkdir -p LABS/jedi-token-detector/sandbox/scripts
cp shared/scripts/flavor_get_token.py LABS/jedi-token-detector/sandbox/scripts/get_token.py
```
...
```bash
LABS/jedi-token-detector/quick-deployment/get_token.py
```
...
API_BASE="$API_BASE" \
python3 LABS/jedi-token-detector/sandbox/scripts/get_token.py
```
...
```bash
mkdir -p LABS/jedi-token-detector/sandbox/lambda-code
cp shared/lambda-code/jedi_python.py LABS/jedi-token-detector/sandbox/lambda-code/jedi_python_token_tracker.py
cp shared/lambda-code/sith_node.js LABS/jedi-token-detector/sandbox/lambda-code/sith_node_token_tracker.js
```
...
| --- | --- | --- |
| `/prod/jedi` | `${PROJECT_NAME}-jedi-python` | `LABS/jedi-token-detector/quick-deployment/jedi_python_token_tracker.py` |
| `/prod/sith` | `${PROJECT_NAME}-sith-node` | `LABS/jedi-token-detector/quick-deployment/sith_node_token_tracker.js` |
...
```bash
touch LABS/jedi-token-detector/sandbox/lambda-code/unused_token_detector.py
```
...
```bash
LABS/jedi-token-detector/quick-deployment/unused_token_detector.py
```

Package and deploy quick reference:

```bash
cd "$LAB_REPO/LABS/jedi-token-detector/quick-deployment"
...
Run the helper and do not call either API route. Wait more than `TOKEN_UNUSED_MINUTES`, then invoke the detector or wait for the schedule.
...
Run the helper again, then call the printed `/prod/jedi` or `/prod/sith` curl command with `x-token-id`.
