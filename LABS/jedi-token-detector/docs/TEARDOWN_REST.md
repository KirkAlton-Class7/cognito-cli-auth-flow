# Unused Token Detector Lab REST Teardown

Use this to tear down a REST version of the unused-token detector lab. This removes the base REST lab plus detector resources.
...
See the production detector REST teardown:

```bash
less ../../../deploy-token-detector/docs/TEARDOWN_REST.md
```
...
export REST_API_ID=$(aws apigateway get-rest-apis --query "items[?name=='${API_NAME}'].id | [0]" --output text --region "$AWS_REGION")
...
aws apigateway delete-rest-api --rest-api-id "$REST_API_ID" --region "$AWS_REGION"
