...
Hands-on lab for extending the Chewbacca Cognito auth flow with token-use telemetry and unused-token detection.<br>
Use the ready code path [here](../../deploy-token-detector/README.md) if you want the concise runbook and packaged code.<br><br>
...
| Document | Use |
| --- | --- |
| [Full Lab](docs/jedi-token-detector-lab.md) | Guided manual editing path with AWS console and CLI validation |
| [REST Teardown](docs/TEARDOWN_REST.md) | Remove REST lab resources plus detector resources |
| [HTTPS Teardown](docs/TEARDOWN_HTTPS.md) | Remove HTTPS lab resources plus detector resources |
| [Token Detector Runbook](../../deploy-token-detector/README.md) | Concise path with finished code |
...
Start with the [full lab](docs/jedi-token-detector-lab.md). Build the base auth flow first with either:
