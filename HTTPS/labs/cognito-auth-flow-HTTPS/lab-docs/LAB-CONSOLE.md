...
Build the Cognito Auth Flow HTTP API lab in the AWS Console, then practice MFA enrollment, Cognito challenge responses, token helper scripts, and protected Jedi/Sith route validation.
...
| Dependency | Requirement |
| --- | --- |
| AWS account and region | Create the HTTPS lab resources in the intended account and region. |
...
| [`TEARDOWN_HTTPS.md`](TEARDOWN_HTTPS.md) | Lab teardown for resources created by this lab. |
