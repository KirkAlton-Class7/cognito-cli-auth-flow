# Cognito Auth Flow - HTTPS Lab - Console
...
This lab teaches the HTTPS/HTTP API version of the Cognito auth flow with a Console-first build path. CLI commands are included for exports, validation, authentication, and protected route testing. It keeps the architecture intentionally small: one user pool, one secret-bearing app client for `SECRET_HASH`, one optional public helper app client, two Lambda functions, one HTTP API, and a Cognito JWT authorizer.
