...
Hands-on lab for building and validating the HTTPS/HTTP API implementation of the Cognito auth flow. This lab keeps the deeper explanation, visual checkpoints, and practice-oriented flow separate from the lean runbooks.

This lab isolates the authentication mechanics so you can see how Cognito behaves before putting the pattern inside a larger application. You will build an HTTP API protected by a Cognito JWT authorizer, create a Chewbacca test user, enroll software-token MFA, use token helper scripts, and validate protected Jedi/Sith Lambda routes.

## Choose A Lab Path

| Document | Use |
| --- | --- |
| [CLI Lab With Environment File](lab-docs/LAB-CLI.md) | Build most resources with AWS CLI commands, then validate the full Cognito auth flow and protected HTTP API routes. |
| [Console Lab With Environment File](lab-docs/LAB-CONSOLE.md) | Build most resources through the AWS Console, using CLI commands for packaging, authentication, validation, and environment-backed resource values. |
...
| [HTTPS Lab Teardown](lab-docs/TEARDOWN_HTTPS.md) | Remove only the base HTTPS auth-flow resources built in this lab. |
...
| --- | --- | --- |
| `/prod/jedi` | Python | Validates the Python Lambda route after JWT authorization succeeds |
| `/prod/sith` | Node.js | Validates the Node.js Lambda route after JWT authorization succeeds |
...
| [lab-docs/TEARDOWN_HTTPS.md](lab-docs/TEARDOWN_HTTPS.md) | Lab-specific teardown for resources created by this lab. |
...
You are ready to leave this HTTPS lab when you can explain the full flow without looking:
