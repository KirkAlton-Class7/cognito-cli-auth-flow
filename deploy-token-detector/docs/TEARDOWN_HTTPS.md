...
Remove the token detector HTTPS deployment resources, then verify that add-on, identity, API, logging, alerting, and IAM lookups return deleted or empty results.
...
| File | Use |
| --- | --- |
| [`../env.example`](../env.example) | Environment template showing token-detector values required by teardown. |
| [`../README.md`](../README.md) | Token detector document map. |
| [`deploy-token-detector-runbook.md`](deploy-token-detector-runbook.md) | Runbook that creates the add-on resources removed here. |
| [`../../REST/docs/RUNBOOK-CLI.md`](../../REST/docs/RUNBOOK-CLI.md) | Base REST deployment runbook referenced when tearing down REST resources. |
| [`../../HTTPS/docs/RUNBOOK-CLI.md`](../../HTTPS/docs/RUNBOOK-CLI.md) | Base HTTPS deployment runbook referenced when tearing down HTTPS resources. |
